...
struct SecureStorage {
    ...
    selected_hosted_model: Option<String>,
}
...
pub async fn secure_storage_save(app: AppHandle, items: Vec<StorageItem>) -> Result<(), String> {
    ...
    for item in items {
        match item.key.as_str() {
            "attune_license_key" | "pluely_license_key" => {
                storage.license_key = Some(item.value)
            }
            "attune_instance_id" | "pluely_instance_id" => {
                storage.instance_id = Some(item.value)
            }
            "selected_hosted_model" | "selected_pluely_model" => {
                storage.selected_hosted_model = Some(item.value)
            }
...
pub async fn secure_storage_get(app: AppHandle) -> Result<StorageResult, String> {
    ...
    Ok(StorageResult {
        ...
        selected_hosted_model: storage.selected_hosted_model,
    })
}
...
pub async fn secure_storage_remove(app: AppHandle, keys: Vec<String>) -> Result<(), String> {
    ...
    for key in keys {
        match key.as_str() {
            "attune_license_key" | "pluely_license_key" => storage.license_key = None,
            "attune_instance_id" | "pluely_instance_id" => storage.instance_id = None,
            "selected_hosted_model" | "selected_pluely_model" => {
                storage.selected_hosted_model = None
            }
...
pub async fn validate_license_api(_app: AppHandle) -> Result<ValidateResponse, String> {
    // License check bypassed: always report active (paywall removed)
    Ok(ValidateResponse {
        is_active: true,
