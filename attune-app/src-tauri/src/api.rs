...
#[tauri::command]
pub async fn deepgram_text_to_speech(
    api_key: String,
    model: String,
    text: String,
) -> Result<String, String> {
    ...
    if text.is_empty() {
        return Err("Text is required for text-to-speech".to_string());
    }
    ...
    if !status.is_success() {
        ...
        if let Ok(error_json) = serde_json::from_str::<serde_json::Value>(&error_text) {
            if let Some(message) = error_json
                .get("message")
                .and_then(|m| m.as_str())
                .or_else(|| error_json.get("err_msg").and_then(|m| m.as_str()))
            {
                return Err(format!("TTS failed ({}): {}", status, message));
            }
        }

        return Err(format!("TTS failed ({}): {}", status, error_text));
    }
    ...
    if bytes.is_empty() {
        return Err("Deepgram returned empty audio".to_string());
    }
