...
/** When set (e.g. system audio session), global screenshot shortcut uses this instead of chat completion. */
let globalScreenshotPriorityCallback: (() => void | Promise<void>) | null =
  null;
...
export const useGlobalShortcuts = () => {
  ...
  const registerPriorityScreenshotCallback = useCallback(
    (callback: (() => void | Promise<void>) | null) => {
      globalScreenshotPriorityCallback = callback;
    },
    []
  );
      ...
      try {
        ...
        const unlistenScreenshot = await listen("trigger-screenshot", () => {
          ...
          const screenshotHandler =
            globalScreenshotPriorityCallback ?? globalScreenshotCallback;
          if (screenshotHandler) {
            try {
              Promise.resolve(screenshotHandler())
                .catch((error) => {
