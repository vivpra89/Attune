...
import { ATTACH_SCREENSHOT_TO_COMPLETION_EVENT, MAX_FILES } from "@/config";
...
export const useCompletion = () => {
  ...
  // System audio screenshot → same image on the main ask bar (paperclip attachments + optional input hint)
  useEffect(() => {
    const onAttachScreenshot = (e: Event) => {
      const detail = (e as CustomEvent<{ base64: string; fillInputHint?: boolean }>)
        .detail;
      const base64 = detail?.base64;
      if (!base64) return;

      setState((prev) => {
        if (prev.attachedFiles.length >= MAX_FILES) {
          return prev;
        }
        const attachedFile: AttachedFile = {
          id: `${Date.now()}-sys-screenshot`,
          name: `screenshot_${Date.now()}.png`,
          type: "image/png",
          base64,
          size: base64.length,
        };
        const fillHint = detail.fillInputHint !== false;
        const nextInput =
          fillHint && !prev.input.trim()
            ? "(Screenshot attached — add your message and press Enter) "
            : prev.input;
        return {
          ...prev,
          attachedFiles: [...prev.attachedFiles, attachedFile],
    ...
    window.addEventListener(
      ...
      onAttachScreenshot as EventListener
    );
    return () =>
  ...
  useEffect(() => {
    ...
    const handleStorageChange = async (e: StorageEvent) => {
      if (e.key === "attune-conversation-selected" && e.newValue) {
        try {
        ...
        if (!hasPermission) {
          ...
          if (!hasPermissionNow) {
            setState((prev) => ({
              ...prev,
              error:
                "Screen Recording permission required. Please enable it by going to System Settings > Privacy & Security > Screen & System Audio Recording. If you don't see Attune in the list, click the '+' button to add it. If it's already listed, make sure it's enabled. Then restart the app.",
            }));
