...
// Sensitivity presets for simpler UX (system audio = meetings/calls)
const SENSITIVITY_PRESETS = {
  ...
  system: {
    ...
    label: "System audio",
    description: "Best for meetings, Zoom, and call audio",
  },
} as const;
      ...
      {isOpen && (
        <div className="px-3 pb-3 space-y-4">
          ...
          <div className="space-y-3">
            ...
            {/* Max Duration - streaming (non-VAD) mode stops the pipeline at this limit */}
            {!vadConfig.enabled && (
              <div className="space-y-2">
                <Label className="text-xs font-medium flex items-center justify-between">
                  <span>Max streaming duration</span>
                  <span className="text-muted-foreground font-normal tabular-nums">
                    {vadConfig.max_recording_duration_secs >= 3600
                      ? "60 min"
                      : vadConfig.max_recording_duration_secs % 60 === 0
                        ? `${vadConfig.max_recording_duration_secs / 60} min`
                        : `${(vadConfig.max_recording_duration_secs / 60).toFixed(1)} min`}
                  </span>
                </Label>
                <Slider
                  value={[
                    Math.min(
                      60,
                      Math.max(1, vadConfig.max_recording_duration_secs / 60)
                    ),
                  ]}
                  onValueChange={([value]) =>
                    onUpdateVadConfig({
                      ...vadConfig,
                      max_recording_duration_secs: Math.min(
                        3600,
                        Math.max(60, Math.round(value * 60))
                      ),
                    })
                  }
                  min={1}
                ...
                <p className="text-[10px] text-muted-foreground leading-snug">
                  In streaming mode, capture ends at this limit (backend max 1 hour).
                  Increase for long meetings; pause/resume starts a fresh timer.
                </p>
              </div>
            )}
          </div>
